...
import { advancedSearchCustomers } from "@/api/customers";
import { useDebouncedValue } from "@/hooks/useDebouncedValue";

export default function AdvancedSearchDialog({ open, onOpenChange, studioId, onSelectCustomer }) {
  ...
  // Matched server-side so the search covers every customer, not just the
  // first page the API would return.
  const debouncedFilters = useDebouncedValue(filters, 250);
  const { data: filteredResults = [] } = useQuery({
    queryKey: ['customerAdvancedSearch', studioId, debouncedFilters],
    queryFn: () => advancedSearchCustomers(studioId, debouncedFilters),
